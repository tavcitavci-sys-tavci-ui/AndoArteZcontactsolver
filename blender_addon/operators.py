...
def _get_settings(context):
    return getattr(context.scene, "artezbuild_ppf", None)
...
class ARTEZBUILD_OT_ppf_run(bpy.types.Operator):
    bl_idname = "artezbuild.ppf_run"
    ...
    def modal(self, context, event):
        ...
        try:
            ...
            self.report({"ERROR"}, f"PPF step failed: {exc}")
            self._stop(context)
            return {"CANCELLED"}
...
class ARTEZBUILD_OT_ppf_stop(bpy.types.Operator):
    bl_idname = "artezbuild.ppf_stop"
    bl_label = "Stop PPF"
    bl_options = {"REGISTER"}
